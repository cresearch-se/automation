...
def compare_page(pid, legacy, gcc):
    ...
    # ── 5. Left nav comparison ────────────────────────────────────────────────
    legacy_left_nav = legacy.get("left_nav", [])
    gcc_left_nav    = gcc.get("left_nav", [])
    legacy_nav_titles = set(t.strip() for t, _ in legacy_left_nav if t.strip())
    gcc_nav_titles    = set(t.strip() for t, _ in gcc_left_nav    if t.strip())
    has_legacy_nav = bool(legacy_nav_titles)
    has_gcc_nav    = bool(gcc_nav_titles)

    if has_legacy_nav and has_gcc_nav:
        missing_from_gcc    = sorted(legacy_nav_titles - gcc_nav_titles)
        missing_from_legacy = sorted(gcc_nav_titles - legacy_nav_titles)
        if missing_from_gcc:
            issues.append(f"Left nav items in legacy MISSING from GCC: {missing_from_gcc}")
        if missing_from_legacy:
            info.append(f"Left nav has extra items in GCC not in legacy: {missing_from_legacy}")
        if not missing_from_gcc:
            info.append(f"Left nav matches — {len(gcc_nav_titles)} item(s) ✓")
    elif not has_legacy_nav and has_gcc_nav:
        info.append(f"Left nav present in GCC only — legacy had no left nav (N/A)")
    elif has_legacy_nav and not has_gcc_nav:
        issues.append("Left nav MISSING on GCC site — legacy had a left nav")
    else:
        info.append("No left nav on either legacy or GCC (N/A)")
...
def generate_report(results, legacy_meta, gcc_meta, output_path):
    ...
    for pid, r in sorted_items:
        ...
        l_nav = set(t.strip() for t,_ in l.get("left_nav",[]) if t.strip())
        g_nav = set(t.strip() for t,_ in g.get("left_nav",[]) if t.strip())
        if l_nav and g_nav:
            nav_ok = '<span style="color:#1D7A6B;font-weight:600">MATCH</span>' if not (l_nav - g_nav) else '<span style="color:#A32D2D;font-weight:600">FAIL</span>'
        elif not l_nav and g_nav:
            nav_ok = '<span style="color:#888">N/A</span>'
        elif l_nav and not g_nav:
...
<div class="rules">
  ...
  Left nav: compared if both sides have it (MATCH/FAIL), N/A if legacy has none, MISSING if GCC has none &nbsp;|&nbsp;
