# PSEUDOCODE / PLAN:
# - Locate the print loop inside _print_diffs_for_sheet that prints each diff row.
# - For each column name (colname), produce a no-space version:
#     colname_nospace = "".join(str(colname).split())
#   This removes all whitespace characters.
# - Use colname_nospace in the formatted print instead of colname.
# - Keep column widths and all other behavior unchanged.
...
def _compare_rows_by_headers(
    ...
    Also skip comparisons for the columns the user requested:
      - "LY - Total Teamwork"
      - "LY - Total Crit/Ast/Ovr (Unique)" (and variants)
      - "Change Teamwork"
      - "Change" / "Total change"
      - "Change Total" (exact header requested)
    The skip logic is robust to small header variations.
    """
    diffs = []
    for h in headers:
        ...
        # Skip exact "Change Total" header (normalized -> "changetotal")
        if nh == "changetotal":
            continue

        if nh == "lytotalteamwork":
            continue

        if nh == "changeteamwork":
            continue

        if nh == "lytotalcritastovrunique":
            continue

        # 2) "LY - Total Crit/Ast/Ovr (Unique)" variants: contains 'ly' and 'total' and one of crit/ast/ovr/unique
        if "ly" in nh and "total" in nh and any(tok in nh for tok in ("crit", "ast", "ovr", "unique")):
            continue

        # 3) "Change Teamwork" variants: contains 'change' and 'teamwork'
        if "change" in nh and "teamwork" in nh:
            continue

        # 4) Generic 'change' or 'totalchange' exact-ish matches (avoid overly broad skips)
...
def _print_diffs_for_sheet(sheet: str, file1_name: str, file2_name: str, diffs_by_key: Dict[str, List[Tuple[str, Any, Any]]]) -> None:
    ...
    for key in sorted(diffs_by_key.keys()):
        ...
        for idx, (colname, v1, v2) in enumerate(rows):
            ...
            # Remove all whitespace from the column name for printing
            colname_nospace = "".join(str(colname).split())
            print(f"{key_display:<{w_key}} | {colname_nospace:<{w_col}} | {v1:<{w_val}} | {v2:<{w_val}}")
    print("-" * len(header_line))


def test_compare_teamwork_profitability_workbooks():
    """
    Compare Teamwork Profitability workbooks (sheet-by-sheet) using same logic as the principals test.
    Files are resolved from tests/TeamworkDB/fixtures and compared across matching sheets.
    """
    base = Path(__file__).parent
    fixtures_dir = base / "fixtures"

    file1 = fixtures_dir / "Teamwork Profitability - Deflated - fix 20260127_2024.xlsx"
    file2 = fixtures_dir / "Teamwork Profitability - Deflated_2024.xlsx"
    ...
    for sheet in common_sheets:
        ...
        if officer_header:
            ...
            if not key_header:
                # if headers shorter than expected choose first header
        ...
        # collect diffs per key for pretty printing
        ...
        # pretty print diffs for this sheet
